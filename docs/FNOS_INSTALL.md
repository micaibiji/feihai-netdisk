...
- 准备你已有的 PanSou 地址和 PanCheck 兼容检测网站地址。
...
2. 进入 **设置**，填写你已有的 PanSou 地址与检测网站地址，分别点击 **测试并保存**。
3. **资源搜索** 输入片名后能显示 Telegram/网盘结果；失效链接不会展示，检测站异常会单独提示。
4. **网盘账号** 页面按顺序显示 115、百度、夸克、中国移动云盘。
5. **风控中心** 点击检测后会为四个网盘生成检测结果。

健康检查地址为 `http://你的NAS局域网IP:12366/api/health`。正常结果中应包含：

```json
{"status":"ok","name":"飞海网盘","version":"0.4.7","database":true,"strm_writable":true}
```
...
3. 四个网盘也都可以点击 **Token 登录**：115 可填写 Access Token/Cookie，百度填写 Access Token，夸克和移动云盘填写 Token/Cookie。
4. Token 登录不要求输入网盘账号密码；只粘贴从相应网盘官方客户端或官方网页取得的凭证。
5. 授权完成后回到账号页，再运行一次风控检测。
6. 页面只显示“Token 已保存”等脱敏状态；凭证由 NAS 本机密钥加密保存，页面与日志不会回显原文。
