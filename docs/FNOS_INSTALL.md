...
本教程面向飞牛 fnOS 用户，不要求使用命令行。完成后会得到三个服务：飞海网盘管理页、Telegram/网盘聚合搜索服务、四网盘扫码授权网关。
...
1. 首页能看到影视榜单；没填 TMDB Key 时会明确显示演示内容。
...
```json
{"status":"ok","name":"飞海网盘","version":"0.3.2","database":true,"strm_writable":true}
```
...
1. 115、夸克、中国移动云盘点击授权后，会跳转到 NAS 本机的 OpenList 网关完成扫码或登录。
2. 百度网盘填写官方 OAuth 参数后可走官方一键授权；未填写时可使用 OpenList 网关。
3. 授权完成后回到账号页，再运行一次风控检测。
4. 页面只显示脱敏账号信息；凭证由 NAS 本机密钥加密保存。
